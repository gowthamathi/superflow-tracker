...
<div class="wrap">
  ...
  <div class="note" style="margin-bottom: 12px;" id="info-note">
    <b>Cached dashboard.</b> Date selector filters this cached data client-side — no live API calls, no fetch errors. Headline KPIs, Languages table and Trends charts respect the selected date range (from daily account/language totals). Campaigns, Themes, Variants and Creatives tabs, plus per-creative counts, CTR and win-rate, reflect the full accumulated dataset (per-ad daily history isn't cached). To refresh: ask Claude to rebuild the snapshot. Production deploy refreshes automatically each morning.
  </div>
...
const AI_FX_KEYWORDS = ["kbh","tractor beam","red marker","phone axe"," axe","keyboard hammer"," hammer","wrecking","drill","pyro","lightning","blackhole","black hole","trex","t-rex","energy burst"," energy","steamroller","hydraulic","nitrogen","cosmic"," 404","chalkboard","cursor trash","camerazoom","thanos","roadroller","blowtorch","chainsaw"];
...
function isAI(name) {
  ...
  // "- SF -" naming convention (newer AI creatives)
  if (/- sf -/i.test(name)) return true;
...
function classify(name) {
  ...
  const subs = [];
  // Pattern Interrupt detection + sub-categories
  const isPI = PI_TOKENS.some(k => n.includes(k)) || hasWordToken(name, ['pi', 'pai']) || AI_FX_KEYWORDS.some(k => n.includes(k));
  if (isPI) {
    subs.push('Pattern Interrupt');
    if (n.includes('phone') || n.includes('phone axe')) subs.push('PI: Phone FX');
    else if (n.includes('keyboard') || n.includes('kbh') || n.includes('final kbh')) subs.push('PI: Keyboard FX');
    else if (/bulldozer|tank|steamroller|hydraulic|trex|t-rex/.test(n)) subs.push('PI: Vehicle/Heavy FX');
    else subs.push('PI: Other FX');
  }
...
// Per-language money summary for a date range, from daily trend_by_language rows.
// This is the date-accurate source for the Languages table's spend/purchases/
// installs/CAC/CPI/Pay columns (per-ad daily history isn't cached).
function computeLangTrendSummaries(since, until) {
  const res = {};
  for (const L in (DATA.trend_by_language || {})) {
    const rows = filterTrend(DATA.trend_by_language[L], since, until);
...
function renderKPIs(summary, periodLabel, fullRange) {
  ...
  const kpis = [
    ...
    {label:'Win Rate', value:winRate, sub:(fullRange?'':'all-time · ')+winCount+'/'+CURRENT_ADS.length+' ≤₹300',
     cls: parseFloat(winRate)>=30?'target-ok':(parseFloat(winRate)>=15?'target-warn':'target-bad')},
    {label:'Creatives', value:fNum(CURRENT_ADS.length), sub:fullRange?'Active period':'All-time total'}
  ];
...
// money = date-filtered account trend summary (spend/purchases/installs/cac/cpi/pay).
// ads  = full-aggregate ad summary (per-ad columns: c2i, ctr, total_ads).
function renderLanguageTable(langs, money, ads) {
  const tbody = document.querySelector('#language-table tbody');
  const tfoot = document.querySelector('#language-table tfoot');
  const totalSpend = money.total_spend, totalPurch = money.total_purchases;
  const baseCpi = money.overall_cpi, basePay = money.overall_pay, baseC2i = ads.overall_c2i;
  ...
  tfoot.innerHTML = `<tr>
    ...
    <td class="r">100%</td>
    <td class="r">${fNum(money.total_installs)}</td>
    <td class="r"><span class="cell bg-grey">${fPct(baseC2i)}</span></td>
    <td class="r"><span class="cell bg-grey">${fPct(basePay)}</span></td>
    <td class="r"><span class="${cellClass(money.overall_cac, null, 'cac')}">${fINR(money.overall_cac)}</span></td>
    <td class="r"><span class="cell bg-grey">${fINR(money.overall_cpi)}</span></td>
    <td class="r">${ads.overall_ctr ? ads.overall_ctr.toFixed(2)+'%' : '—'}</td>
    <td class="r">${ads.total_ads}</td>
    <td class="r">${langs.reduce((s,L)=>s+L.ads_under_300_cac,0)}</td>
    <td class="r">${ads.total_ads > 0 ? (100*langs.reduce((s,L)=>s+L.ads_under_300_cac,0)/ads.total_ads).toFixed(1)+'%' : '—'}</td>
  </tr>`;
}
...
function renderAll() {
  destroyAllCharts();
  const fullRange = (DATE_FILTER.since === SNAP_FIRST && DATE_FILTER.until === SNAP_LAST);

  // Per-ad tabs (Campaigns, Themes, Variants, Creatives) and per-ad columns
  // (CTR, creative counts, win-rate) use the full accumulated aggregate —
  // per-ad daily history isn't cached, so these can't be sliced by date.
  CURRENT_ADS = DATA.ads;
  CURRENT_SUMMARY = computeSummary(CURRENT_ADS);

  // Date-range-accurate money metrics come from the daily trend tables, which
  // accumulate for the whole timeline (this is what the KPIs / Languages show).
  const trendRows = filterTrend(DATA.trend_account, DATE_FILTER.since, DATE_FILTER.until);
  const trendSummary = computeTrendSummary(trendRows);
  const langTrend = computeLangTrendSummaries(DATE_FILTER.since, DATE_FILTER.until);

  const periodLabel = fullRange
    ? 'Full snapshot' : (DATE_FILTER.since + ' → ' + DATE_FILTER.until);
  renderKPIs(trendSummary, periodLabel, fullRange);

  // Languages: money columns from date-filtered trend, per-ad columns from full aggregate.
  const langsFull = aggregateLanguages(CURRENT_ADS);
  const langs = langsFull.map(Lf => {
    const t = langTrend[Lf.language] || {spend:0, purchases:0, installs:0, cac:null, cpi:null, pay:null};
    return {...Lf, spend:t.spend, purchases:t.purchases, installs:t.installs, cac:t.cac, cpi:t.cpi, pay:t.pay};
  });
  renderLanguageTable(langs, trendSummary, CURRENT_SUMMARY);
...
(function initDates() {
  if (SNAPSHOT.trend_account && SNAPSHOT.trend_account.length) {
    ...
    setBadge(`Cache · ${SNAP_FIRST} → ${SNAP_LAST} (full snapshot)`, 'live');
  }
})();
